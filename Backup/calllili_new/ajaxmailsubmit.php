...
$conn = $dbConObj->dbConnect();
 $firstBase64Image=    isset($_POST["firstBase64Image"]) ? $_POST["firstBase64Image"] : "";
	 	 $secondBase64Image=    isset($_POST["secondBase64Image"]) ? $_POST["secondBase64Image"] : "";
	     $Platform =    isset($_REQUEST["Platform"]) ? $_REQUEST["Platform"] : "web";
...
//    print_r($_POST); die;
  ...
  $temp = "";
    ...
    if (!empty($file_ext)) {
        // Create new filename using BookingID2 and the extracted extension
        $new_file_name = $_POST['BookingID2'] . "_Receipt." . $file_ext;

        // Call your upload function with the new filename
        UploadFile($new_file_name, $file_size, $file_tmp, $file_type);

        // Add the new filename to the files array
        array_push($files2, "uploads/timesheet/" . $new_file_name);
    }
}
...
$fromApp = isset($_POST['fromApp']) ? $_POST['fromApp'] : false;
...
//    $to = "finance@absolute-interpreting.co.uk";
    ...
    multi_attach_mail($to, $subject, $files2, $from, $message, $showTimesheetName,$ShowTimeSheetOnline);
    ...
    if(round($return_milage2) AND round($return_time2)) {
        $goomiles = round($return_milage2);
        $gootime = round($return_time2);
    }
    else {
        $goomiles = round($return_milage1);
        $gootime = round($return_time1);
    }
    ...
    if($_POST['IsClientInclusive']){
        ...
        if($goomiles >= $_POST['AfterHowManyMilesToChargeMileage'] OR is_null($_POST['AfterHowManyMilesToChargeMileage'])){
            ...
            if($_POST['TypeOfTransport']=='Drives'){
    ...
    if (strlen($file_name)) {
        ...
        if (is_array($formats) && in_array($ext, $formats)) {
            if ($file_size < (3 * 1024 * 1024)) {
                ...
                echo "Invalid Image File Size is Max 1 MB";
                $isProceed = false;
                exit;
            }
        } else {
...
function multi_attach_mail_old($to, $subject, $files, $sendermail, $message, $showTimesheetName) {
...
function  multi_attach_mail($to, $subject, $files, $sendermail, $message, $showTimesheetName,$ShowTimeSheetOnline) {
    ...
    try {
        ...
        //Recipients
        $mail->setFrom($sendermail, 'Absolute Interpreting & Translations Ltd');
        $mail->addAddress($to);
   //$mail->addAddress("ali.raza@absolute-interpreting.co.uk");
        //Content
        $mail->isHTML(true);
        ...
        $mail->send();
        return count($files); // Return the count of files attached
    } catch (Exception $e) {
        // Handle exception
